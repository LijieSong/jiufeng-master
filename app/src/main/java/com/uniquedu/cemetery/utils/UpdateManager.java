        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case DOWNLOAD_FINISH:
                    ...
                    notification.contentView.setTextViewText(R.id.down_tv, "下载已完成,点击进行安装");
                    //设置当前值为count
                    nm.notify(notification_id,notification);
                    // 安装文件
                    installApk();
...
//                showDownloadDialog();
    ...
    private void showNotificationDownLoad() {
        ...
        notification = new Notification(R.mipmap.ic_launcher, "丰碑在线下载中...", System.currentTimeMillis());
        ...
        if (isFinish == true){//判断是会否下载完成
            notificationIntent = new Intent(Intent.ACTION_VIEW);//下载完成点击通知栏就是安装
            File apkfile = new File(mSavePath, mHashMap.get("name"));
            if (!apkfile.exists()) {
                return;
            }
            notificationIntent.setDataAndType(Uri.parse("file://" + apkfile.toString()), "application/vnd.android.package-archive");
            nm.cancel(notification_id);
        }else{
            notificationIntent = new Intent(mContext, MainActivity.class);//没有就是跳转主界面
        }
